...
namespace Zlib.Extended
{
    public static class Inflate
    {
        ...
        public static ReturnCode inflateReset(z_stream strm)
        {
        ...
        public static ReturnCode inflateReset2(z_stream strm, int windowBits)
        {
            // get the state
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            ...
            if (windowBits != 0 && (windowBits < 8 || windowBits > 15)) return ReturnCode.Z_STREAM_ERROR;
        ...
        public static ReturnCode inflateInit2(z_stream strm, int windowBits)
        {
            ...
            strm.state = state;
            state.window = null;
            ReturnCode ret = inflateReset2(strm, windowBits);
            if (ret != ReturnCode.Z_OK) strm.state = null;
        ...
        public static ReturnCode inflateInit(z_stream strm)
        {
        ...
        public static ReturnCode inflatePrime(z_stream strm, int bits, int value)
        {
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            if (bits < 0)
            ...
            if (bits > 16 || state.bits + bits > 32) return ReturnCode.Z_STREAM_ERROR;
            value &= (1 << (int)bits) - 1;
            ...
            for (; ; )
            {
                switch (state.mode)
                {
                    ...
                    case inflate_mode.DONE:
                        ret = ReturnCode.Z_STREAM_END;
                        goto inf_leave;
                    case inflate_mode.BAD:
                        ret = ReturnCode.Z_DATA_ERROR;
                        goto inf_leave;
                    case inflate_mode.MEM: return ReturnCode.Z_MEM_ERROR;
                    case inflate_mode.SYNC: return ReturnCode.Z_STREAM_ERROR;
                    default: return ReturnCode.Z_STREAM_ERROR;
                } // switch(state.mode)
            } // for(; ; )
        ...
        public static ReturnCode inflateEnd(z_stream strm)
        {
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            state.window = null;
        ...
        public static ReturnCode inflateSetDictionary(z_stream strm, byte[] dictionary, uint dictLength)
        {
            // check state
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;

            inflate_state state = (inflate_state)strm.state;
            if (state.wrap != 0 && state.mode != inflate_mode.DICT) return ReturnCode.Z_STREAM_ERROR;
            ...
            if (state.mode == inflate_mode.DICT)
            {
                ...
                if (id != state.check) return ReturnCode.Z_DATA_ERROR;
            }
        ...
        public static ReturnCode inflateGetHeader(z_stream strm, gz_header head)
        {
            // check state
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            if ((state.wrap & 2) == 0) return ReturnCode.Z_STREAM_ERROR;
        ...
        public static ReturnCode inflateSync(z_stream strm)
        {
            ...
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            if (strm.avail_in == 0 && state.bits < 8) return ReturnCode.Z_BUF_ERROR;
            ...
            if (state.have != 4) return ReturnCode.Z_DATA_ERROR;
        ...
        public static ReturnCode inflateSyncPoint(z_stream strm)
        {
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            return (state.mode == inflate_mode.STORED && state.bits == 0) ? ReturnCode.Z_STREAM_END : ReturnCode.Z_OK;
        }
        ...
        public static ReturnCode inflateCopy(z_stream dest, z_stream source)
        {
            // check input
            if (dest == null || source == null || source.state == null) return ReturnCode.Z_STREAM_ERROR;
        ...
        public static ReturnCode inflateUndermine(z_stream strm, bool subvert)
        {
            if (strm == null || strm.state == null) return ReturnCode.Z_STREAM_ERROR;
            inflate_state state = (inflate_state)strm.state;
            state.sane = !subvert;
...
#else
            state.sane = true;
            return ReturnCode.Z_DATA_ERROR;
#endif
        }
