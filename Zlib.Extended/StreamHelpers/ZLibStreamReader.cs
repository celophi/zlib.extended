...
namespace Zlib.Extended.StreamHelpers
{
    public class ZLibStreamReader : Stream, IDisposable
    {
        Stream baseStream;
        zlib.z_stream zstream;

        public ZLibStreamReader(Stream stream) : this(stream, 0x1000) { }
        ...
        public override bool CanRead { get { return baseStream != null && baseStream.CanRead; } }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                if (zstream.avail_in == 0)
                {
                    ...
                    zstream.next_in = 0;
                    zstream.avail_in = (uint)baseStream.Read(zstream.in_buf, 0, zstream.in_buf.Length);
                }
                int ret = zlib.inflate(zstream, zlib.Z_PARTIAL_FLUSH);
                if (ret == zlib.Z_STREAM_END)
                {
                    if (zstream.avail_in != 0) throw new Exception("Extra compressed data");
                    break;
                }
                if (ret != zlib.Z_OK) throw new Exception(zstream.msg != null && zstream.msg.Length > 0 ? zstream.msg : "Decompression error");
