...
namespace Zlib.Extended
{
    public static class Deflate
    {
        ...
        public class deflate_state //internal_state 
        {
            ...
            public FlushValue last_flush;           // value of flush param for previous deflate call
        ...
        delegate block_state compress_func(deflate_state s, FlushValue flush);
        ...
        public static ReturnCode deflateReset(z_stream strm)
        {
            ...
            s.last_flush = FlushValue.Z_NO_FLUSH;
        ...
        public static ReturnCode deflateParams(z_stream strm, int level, int strategy)
        {
            ...
            if ((strategy != s.strategy || func != configuration_table[level].func) && strm.total_in != 0) // Flush the last buffer:
                err = deflate(strm, FlushValue.Z_BLOCK);
        ...
        public static ReturnCode deflate(z_stream strm, FlushValue flush)
        {
            FlushValue old_flush; // value of flush param for previous deflate call

            if (strm == null || strm.state == null || flush > FlushValue.Z_BLOCK || flush < 0) return ReturnCode.Z_STREAM_ERROR;
            deflate_state s = (deflate_state)strm.state;

            if (strm.out_buf == null || (strm.in_buf == null && strm.avail_in != 0) || (s.status == FINISH_STATE && flush != FlushValue.Z_FINISH))
            {
            ...
            if (s.pending != 0)
            {
                ...
                if (strm.avail_out == 0)
                {
                    ...
                    s.last_flush = FlushValue.Z_INVALID;
                    return ReturnCode.Z_OK;
                }
            ...
            else if (strm.avail_in == 0 && flush <= old_flush && flush != FlushValue.Z_FINISH)
            {
                strm.msg = Zutil.ErrorMessages[ReturnCode.Z_BUF_ERROR];
                return ReturnCode.Z_BUF_ERROR;
            }
            ...
            if (strm.avail_in != 0 || s.lookahead != 0 || (flush != FlushValue.Z_NO_FLUSH && s.status != FINISH_STATE))
            {
                ...
                {
                    if (strm.avail_out == 0) s.last_flush = FlushValue.Z_INVALID; // avoid BUF_ERROR next call, see above
                    return ReturnCode.Z_OK;
                ...
                {
                    if (flush == FlushValue.Z_PARTIAL_FLUSH) Trees._tr_align(s);
                    else if (flush != FlushValue.Z_BLOCK)
                    { // FULL_FLUSH or SYNC_FLUSH
                        Trees._tr_stored_block(s, null, 0, 0);
                        // For a full flush, this empty block will be recognized
                        // as a special marker by inflate_sync().
                        if (flush == FlushValue.Z_FULL_FLUSH)
                        {
                    ...
                    if (strm.avail_out == 0)
                    {
                        s.last_flush = FlushValue.Z_INVALID; // avoid BUF_ERROR at next call, see above
                        return ReturnCode.Z_OK;
                    }
                }
            }
            ...
            if (flush != FlushValue.Z_FINISH) return ReturnCode.Z_OK;
        ...
        static block_state deflate_stored(deflate_state s, FlushValue flush)
        {
            ...
            for (; ; )
            {
                ...
                if (s.lookahead <= 1)
                {
                    ...
                    fill_window(s);
                    if (s.lookahead == 0 && flush == FlushValue.Z_NO_FLUSH) return block_state.need_more;
            ...
            Trees._tr_flush_block(s, s.block_start >= 0 ? s.window : null, s.block_start >= 0 ? s.block_start : 0,
                (uint)((int)s.strstart - s.block_start), flush == FlushValue.Z_FINISH ? 1 : 0);
            s.block_start = (int)s.strstart;
            flush_pending(s.strm);
            //Tracev((stderr,"[FLUSH]"));
            if (s.strm.avail_out == 0) return flush == FlushValue.Z_FINISH ? block_state.finish_started : block_state.need_more;

            return flush == FlushValue.Z_FINISH ? block_state.finish_done : block_state.block_done;
        }
        ...
        static block_state deflate_fast(deflate_state s, FlushValue flush)
        {
            ...
            for (; ; )
            {
                ...
                {
                    fill_window(s);
                    if (s.lookahead < MIN_LOOKAHEAD && flush == FlushValue.Z_NO_FLUSH) return block_state.need_more;
                    if (s.lookahead == 0) break; // flush the current block
                }
        ...
        static block_state deflate_slow(deflate_state s, FlushValue flush)
        {
        ...
        static block_state deflate_rle(deflate_state s, FlushValue flush)
        {
            ...
            for (; ; )
            {
                ...
                {
                    fill_window(s);
                    if (s.lookahead < MAX_MATCH && flush == FlushValue.Z_NO_FLUSH) return block_state.need_more;
                    if (s.lookahead == 0) break; // flush the current block
                }
        ...
        static block_state deflate_huff(deflate_state s, FlushValue flush)
        {
            bool bflush;                // set if current block must be flushed
            ...
            for (; ; )
            {
                ...
                if (s.lookahead == 0)
                {
                    ...
                    if (s.lookahead == 0)
                    {
                        if (flush == FlushValue.Z_NO_FLUSH)
